...
for i in range(n):
    ...
    fx0 = Fx.subs(x, x0).evalf()
    fx1 = Fx.subs(x, x1).evalf()
    fx2 = Fx.subs(x, x2).evalf()
    # Finding H 0 & 1
    H0 = x1 - x0
    H1 = x2 - x1
    # Finding Delta 0 & 1 
    ...
    if abs(DeltaPos) > abs(DeltaNeg):
        x3 = x2 + ((- 2 * C) / DeltaPos)
    else:
        x3 = x2 + ((- 2 * C) / DeltaNeg)
    Error = abs(((x3 - x2) / x3) * 100)
    print(
        "     {0:2d}     |  {1:.5f} | {2:.5f} | {3:.5f} | {4:.5f} | {5:.4f} | {6:.4f} | {7:.4f} | {8:.4f} | {9:.6f} \n"
...
# Finally displaying the output with the value of every rule that was needed in order to get the final answer and the
# percentage of error after each iteration
